...
def main():
    ...
    plant.AddJoint(PrismaticJoint("cart_track", plant.world_frame(), cart.body_frame(), np.array([1., 0., 0.])))
    pole = plant.AddRigidBody("Pole", SpatialInertia.SolidCylinderWithMass(pole_mass, 0.3, pole_length, np.array([0., -1., 0.])))
    plant.AddJoint(RevoluteJoint("cart-pole",cart.body_frame(), pole.body_frame(), np.array([0., 0., 1.])))
    plant.Finalize()
    ...
    for i in range(1, N):
        ...
        # x accel
        prog.AddConstraint(eq(qd[i][0], qd[i-1][0] + (F[i-1] + pole_mass * sin(q[i-1][1]) * (pole_length * pow(qd[i-1][1],2) + g * cos(q[i-1][1]))) / (cart_mass + pole_mass * pow(sin(q[i-1][1]), 2)) * dt))

        # angular accel
        prog.AddConstraint(eq(qd[i][1], (qd[i-1][1] + (-F[i-1] * cos(q[i-1][1]) - pole_mass * pole_length * pow(qd[i-1][1], 2) * cos(q[i-1][1]) * sin(q[i-1][1]) - (cart_mass + pole_mass) * g * sin(q[i-1][1])) / (pole_length * (cart_mass + pole_mass * pow(sin(q[i-1][1]),2))) * dt)))
